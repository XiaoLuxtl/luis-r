...
**The Insight:** Chatbot development scalability starts with clean infrastructure separation.
...
**The Action:** I implemented **Nginx as a Reverse Proxy/Intermediary**. I manually configured the Nginx environment to explicitly pass vital headers (X-Forwarded-For, X-Real-IP) and managed the handshake between external web traffic and the internal Docker network.

**The Result:** A rock-solid communication bridge that maintains session integrity and secures the data layer.

![n8n Logic Flow](../src/assets/images/n8n-flow.png)

### Phase 3: The "CURL" Revelation (Data Mapping & Debugging)
**The Problem:** The bot was successfully connected, but response bubbles appeared empty. Logs showed a "200 OK" status, yet no data rendered on the UI.

**The Action:** I used **CURL** via the terminal to intercept and analyze the raw JSON payload.

**The Discovery:** The logic looked for a top-level property, but the actual response was nested within a specific object path (*data.output*).
...
Through this project, I have proven I possess the technical autonomy and analytical depth necessary for a Chatbot Developer:
...
* **Problem-Solving Mindset:** A proactive approach to researching and solving technical "roadblocks."
